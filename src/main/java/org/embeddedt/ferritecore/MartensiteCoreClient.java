...
public class MartensiteCoreClient {
    ...
    public void collectTextures(TextureStitchEvent.Pre event) {
        ...
        for(IResourcePack pack : resourcePacks) {
            try {
                ...
                MartensiteCore.LOGGER.error("Error listing resources", e);
            }
        }
        watch.stop();
        MartensiteCore.LOGGER.info("Texture search took {}", watch);
    }
    ...
    private static Collection<ResourceLocation> getAllResources(IResourcePack pack) throws IOException {
        ...
        if(pack instanceof DefaultResourcePack) {
            ...
            MartensiteCore.LOGGER.warn("Cannot list resources from pack {} ({})", pack.getPackName(), pack.getClass().getName());
        }
